    ...
    return (
        <div className='m-8'>
            ...
            <div className='flex gap-2 flex-wrap'>
                <button
                    className={`py-1 px-2 rounded-md border ${type === 'all' && 'bg-red-600'}`}
                    onClick={() => setType('all')}
                >
                    Все
                </button>
                <button
                    className={`py-1 px-2 rounded-md border ${type === 'Оформлен' && 'bg-red-600'}`}
                    onClick={() => setType('Оформлен')}
                >
                    Оформлен
                </button>
                <button
                    className={`py-1 px-2 rounded-md border ${type === 'В доставке' && 'bg-red-600'}`}
                    onClick={() => setType('В доставке')}
                >
                    В доставке
                </button>
                <button
                    className={`py-1 px-2 rounded-md border ${type === 'Доставлен' && 'bg-red-600'}`}
                    onClick={() => setType('Доставлен')}
                >
                    Доставлен
                </button>
                <button
                    className={`py-1 px-2 rounded-md border ${type === 'Отменен' && 'bg-red-600'}`}
